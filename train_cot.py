"""
Phase 2.8: SOSM Training with Chain-of-Thought
==============================================

Key Changes from Phase 2.9:
- Adds <think>...</think> reasoning demonstrations
- Uses CoTDataGenerator to inject structured reasoning
- Teaches model to plan before generating
- Fixes semantic drift by maintaining global context
"""
import torch
...
def train_cot_sosm(epochs=10, batch_size=32, max_samples=50000, k_schedule=500, cot_ratio=0.6):
    """
    Train SOSM with Chain-of-Thought reasoning.
    """
    print("="*70)
    print("PHASE 2.8: CHAIN-OF-THOUGHT SOSM TRAINING")
    print("="*70)
    ...
    torch.save({
        ...
        'config': config,
        'results': results
    }, output_dir / 'sosm_cot_phase2_8.pt')
    
    with open(output_dir / 'phase2_8_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\n✅ Phase 2.8 Complete!")
    print(f"   Model: results/sosm_cot_phase2_8.pt")
    print(f"   Results: results/phase2_8_results.json")
    
    return results


if __name__ == '__main__':
    results = train_cot_sosm(
        epochs=10,
        batch_size=32,
        max_samples=50000,
        k_schedule=500,
        cot_ratio=0.6  # 60% of data gets CoT augmentation
    )
